                                ...
                                const char *passphrase)
{
    ...
    if (!session->auth.auto_state) {
        session->auth.auto_state =
                SSH_CALLOC(1, sizeof(struct ssh_auth_auto_state_struct));
        if (!session->auth.auto_state) {
            ssh_set_error_oom(session);
            return SSH_AUTH_ERROR;
        }
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_info_request) {
    ...
    session->kbdint->prompts = SSH_CALLOC(nprompts, sizeof(char *));
    ...
    session->kbdint->echo = SSH_CALLOC(nprompts, sizeof(unsigned char));
...
int ssh_userauth_kbdint_setanswer(ssh_session session, unsigned int i,
    ...
    if (session->kbdint->answers == NULL) {
        session->kbdint->answers = SSH_CALLOC(session->kbdint->nprompts, sizeof(char *));
        if (session->kbdint->answers == NULL) {
            ssh_set_error_oom(session);
            return -1;
        }
