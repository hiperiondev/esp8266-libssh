...
struct ssh_buffer_struct *ssh_buffer_new(void)
{
    ...
    buf = SSH_CALLOC(1, sizeof(struct ssh_buffer_struct));
    if (buf == NULL) {
        return NULL;
    }
 ...
 * after being reallocate or freed.
 *
...
static int realloc_buffer(struct ssh_buffer_struct *buffer, size_t needed)
{
    ...
    if (needed > SSH_BUFFER_SIZE_MAX) {
        printf("\n\terror: realloc_buffer [needed: %d / free: %d]\n\t", needed, SSH_BUFFER_SIZE_MAX);
        return -1;
    }
 ...
 *
 * In case the buffer has exceeded 64K in size, the buffer will be reallocate
 * to 64K.
 *
...
int ssh_buffer_reinit(struct ssh_buffer_struct *buffer)
{
    ...
    //TODO: change maximum buffer
    if (buffer->allocated > SSH_BUFFER_SIZE_MAX) {
        int rc;

        /* -1 for SSH_REALLOC_buffer magic */
        rc = realloc_buffer(buffer, SSH_BUFFER_SIZE_MAX - 1);
        if (rc != 0) {
            return -1;
        }
    }
...
/**
 * @brief Ensure the buffer has at least a certain preallocate size.
 *
