...
ssh_channel ssh_channel_new(ssh_session session)
{
    ...
    channel = SSH_CALLOC(1, sizeof(struct ssh_channel_struct));
    if (channel == NULL) {
...
int ssh_channel_select(ssh_channel *readchans, ssh_channel *writechans,
  ...
  rchans = SSH_CALLOC(count_ptrs(readchans) + 1, sizeof(ssh_channel));
  if (rchans == NULL) {
    return SSH_ERROR;
  }

  wchans = SSH_CALLOC(count_ptrs(writechans) + 1, sizeof(ssh_channel));
  if (wchans == NULL) {
    SAFE_FREE(rchans);
    return SSH_ERROR;
  }

  echans = SSH_CALLOC(count_ptrs(exceptchans) + 1, sizeof(ssh_channel));
  if (echans == NULL) {
    SAFE_FREE(rchans);
    SAFE_FREE(wchans);
    return SSH_ERROR;
