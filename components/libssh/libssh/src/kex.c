...
int ssh_set_client_kex(ssh_session session)
{
    ...
    kex_tmp = SSH_REALLOC(kex, kex_len);
    if (kex_tmp == NULL) {
        SSH_FREE(kex);
        ssh_set_error_oom(session);
        return SSH_ERROR;
...
int ssh_generate_session_keys(ssh_session session)
{
    ...
    IV_cli_to_srv = SSH_MALLOC(IV_len);
    IV_srv_to_cli = SSH_MALLOC(IV_len);
    enckey_cli_to_srv = SSH_MALLOC(enckey_cli_to_srv_len);
    enckey_srv_to_cli = SSH_MALLOC(enckey_srv_to_cli_len);
    intkey_cli_to_srv = SSH_MALLOC(intkey_cli_to_srv_len);
    intkey_srv_to_cli = SSH_MALLOC(intkey_srv_to_cli_len);
...
error:
    ...
    if (rc != 0) {
        SSH_FREE(IV_cli_to_srv);
        SSH_FREE(IV_srv_to_cli);
        SSH_FREE(enckey_cli_to_srv);
        SSH_FREE(enckey_srv_to_cli);
        SSH_FREE(intkey_cli_to_srv);
        SSH_FREE(intkey_srv_to_cli);
    }
