        ...
        ssh_auth_callback auth_fn, void *auth_data)
{
    ...
    switch (type) {
        case SSH_KEYTYPE_RSA:
            rsa = SSH_MALLOC(sizeof(mbedtls_pk_context));
            if (rsa == NULL) {
                return NULL;
            }
        ...
        case SSH_KEYTYPE_ECDSA_P521:
            ecdsa = SSH_MALLOC(sizeof(mbedtls_pk_context));
            if (ecdsa == NULL) {
                return NULL;
            }
    ...
    if (ecdsa != NULL) {
        ...
        key->ecdsa = SSH_MALLOC(sizeof(mbedtls_ecdsa_context));
        if (key->ecdsa == NULL) {
            goto fail;
        }
                          ...
                          ssh_string q)
{
    ...
    key->rsa = SSH_MALLOC(sizeof(mbedtls_pk_context));
    if (key->rsa == NULL) {
        return SSH_ERROR;
    }
...
ssh_key pki_key_dup(const ssh_key key, int demote)
{
    ...
    switch(key->type) {
        case SSH_KEYTYPE_RSA: {
            ...
            new->rsa = SSH_MALLOC(sizeof(mbedtls_pk_context));
            if (new->rsa == NULL) {
                goto fail;
            }
        ...
        case SSH_KEYTYPE_ECDSA_P521:
            ...
            new->ecdsa = SSH_MALLOC(sizeof(mbedtls_ecdsa_context));
                                   ...
                                   enum ssh_digest_e hash_type)
{
    ...
    sig = SSH_MALLOC(mbedtls_pk_get_bitlen(privkey) / 8);
    if (sig == NULL) {
        return NULL;
    }
