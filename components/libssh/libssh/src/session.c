...
ssh_session ssh_new(void)
{
    ...
    session = SSH_CALLOC(1, sizeof (struct ssh_session_struct));
    if (session == NULL) {
        return NULL;
    }
...
int ssh_get_pubkey_hash(ssh_session session, unsigned char **hash)
{
    ...
    *hash = NULL;
    ...
    h = SSH_CALLOC(MD5_DIGEST_LEN, sizeof(unsigned char));
    if (h == NULL) {
        return SSH_ERROR;
    }
    ...
    case SSH_PUBLICKEY_HASH_SHA256:
        {
            ...
            h = SSH_CALLOC(1, SHA256_DIGEST_LEN);
            if (h == NULL) {
                rc = -1;
                goto out;
            }
