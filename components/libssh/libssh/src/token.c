...
struct ssh_tokens_st *ssh_tokenize(const char *chain, char separator)
{
    ...
    tokens = SSH_CALLOC(1, sizeof(struct ssh_tokens_st));
    if (tokens == NULL) {
        return NULL;
    }
    ...
    tokens->tokens = SSH_CALLOC(num_tokens + 1, sizeof(char *));
    if (tokens->tokens == NULL) {
        goto error;
    }
...
char *ssh_remove_duplicates(const char *list)
{
    ...
    should_copy = SSH_CALLOC(1, max_len);
    if (should_copy == NULL) {
        goto out;
    }
                                    ...
                                    const char *appended_list)
{
    ...
    concat_len += 2;
    concat = SSH_CALLOC(1, concat_len);
    if (concat == NULL) {
        return NULL;
    }
